...
function generateSlug(title, callbackFunction) {
    $.get('/ajax/blog/generate-slug.php?title=' + title).done(function (xml) {
...
function checkSlug(slug, postId, callbackFunction) {
    let url = '/ajax/blog/check-slug.php?slug=' + slug;
    if (postId) {
        url += '&post_id=' + postId;
    }
...
function loadComments(postId) {
    $.get('/ajax/blog/get-comments.php?post=' + postId).done(function (xml) {
        let comments = "";
...
function postComment(postId, alias, contents, callbackFunction) {
    $.post('/ajax/blog/add-comment.php', {
        'post_id' : postId,
        'alias' : alias,
        'contents' : contents
    }).done(function(xml) {
